import type { GenomeOrganization, RankedGuide } from '../types/crispr'

export function TargetVisualization({ guides, selectedId, experiment, genomeOrganization }: { guides: RankedGuide[]; selectedId?: string; experiment: string; genomeOrganization: GenomeOrganization }) {
  ...
  return (
    <figure className="target-figure">
      ...
      <svg viewBox="0 0 900 190" role="img" aria-labelledby="target-title target-desc">
        ...
        <desc id="target-desc">{genomeOrganization === 'prokaryotic' ? 'A single coding feature is shown. Triangles mark candidate cut locations.' : 'Exons are boxes connected by intron lines. Triangles show candidate cut locations; the selected guide is emphasized.'}</desc>
        {genomeOrganization === 'prokaryotic' ? <>
          <line x1="70" y1="104" x2="830" y2="104" className="intron-line" />
          <rect x="90" y="72" width="720" height="64" rx="4" className="exon-box" />
          <text x="450" y="109">Coding sequence (CDS)</text>
        </> : <>
          <line x1="70" y1="104" x2="830" y2="104" className="intron-line" />
      ...
      <div className="figure-legend"><span><i className="legend-coding" /> {genomeOrganization === 'prokaryotic' ? 'Coding sequence' : 'Coding exon'}</span>{genomeOrganization === 'eukaryotic' && <span><i className="legend-utr" /> Untranslated region</span>}<span><i className="legend-cut" /> Candidate cut</span><span><i className="legend-selected" /> Selected guide</span></div>
    </figure>
  )
}
